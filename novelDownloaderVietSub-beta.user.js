...
; (function () { // eslint-disable-line no-extra-semi
  ...
  */
    ...
    const HISTORY_KEY = 'novelDownloaderHistory';
    ...
    const loadDownloadHistory = () => {
        try {
            ...
            console.warn('Không thể tải lịch sử tải xuống:', error);
            return [];
        }
    };

    const saveDownloadHistory = (entries = []) => {
        try {
            GM_setValue(HISTORY_KEY, entries.slice(0, HISTORY_LIMIT));
        } catch (error) {
            console.warn('Không thể lưu lịch sử tải xuống:', error);
        }
    };

    const addDownloadHistoryEntry = (entry) => {
        if (!entry) return;
        ...
        history.unshift({
            ...
            totalChapters: typeof entry.totalChapters === 'number' ? entry.totalChapters : null,
            successChapters: typeof entry.successChapters === 'number' ? entry.successChapters : null,
    ...
    async function showUI() {
        ...
        const html = [
            ...
            '  Chế độ tải: <select name="downloadMode"><option value="parallel">Song song</option><option value="sequential">Tuần tự</option></select>',
            ...
            '  <input type="button" name="toggle-history" value="Lịch sử tải">',
            ...
            '</div>',
            '<div name="history-panel" class="nd-history-panel" style="display:none;">',
            '  <div class="nd-history-toolbar">',
            '    <span class="nd-history-title">Lịch sử tải gần đây</span>',
            '    <span class="nd-history-actions">',
            '      <button type="button" name="history-clear">Xóa lịch sử</button>',
            '      <button type="button" name="history-close">Đóng</button>',
            '    </span>',
            '  </div>',
            '  <table>',
            '    <thead><tr><th>Thời gian</th><th>Tiêu đề</th><th>Định dạng</th><th>Chương</th><th>Trạng thái</th><th>Tệp</th></tr></thead>',
            '    <tbody></tbody>',
            '  </table>',
            '</div>',
        ].join('');
        ...
        const resolveThreadConfig = () => {
            const base = Math.max(1, Number(Config.thread) || 1);
            const ruleLimit = Storage.rule && Storage.rule.thread ? Storage.rule.thread : null;
            const preferred = ruleLimit && ruleLimit < base ? ruleLimit : base;
            const forced = Config.downloadMode === 'sequential' || Config.addChapterNext || Config.addChapterPrev;
            return forced ? 1 : preferred;
        };
        const syncXhrThreadConfig = () => {
            try {
                if (xhr && xhr.storage && xhr.storage.config && typeof xhr.storage.config.set === 'function') {
                    xhr.storage.config.set('thread', resolveThreadConfig());
                }
            } catch (error) {
                console.warn('Không thể đồng bộ thread với xhr:', error);
            }
        };
        const applyDownloadModeUIState = () => {
            const modeSelect = container.find('select[name="downloadMode"]');
            const threadInput = container.find('input[name="thread"]');
            if (modeSelect.length) {
                modeSelect.val(Config.downloadMode || 'parallel');
            }
            if (!threadInput.length) return;
            if (Config.downloadMode === 'sequential') {
                threadInput.attr('disabled', 'disabled').attr('data-disabled-by-mode', 'true');
                threadInput.attr('title', 'Chế độ tuần tự sử dụng 1 luồng để đảm bảo thứ tự chương.');
            } else {
                if (threadInput.attr('data-disabled-by-mode')) {
                    threadInput.removeAttr('data-disabled-by-mode');
                }
                if (!threadInput.is('[raw-disabled="disabled"]')) {
                    threadInput.attr('disabled', null);
                }
                threadInput.attr('title', '');
            }
            syncXhrThreadConfig();
        };
        Storage.getEffectiveThread = resolveThreadConfig;
        const historyPanel = container.find('[name="history-panel"]');
        const historyTableBody = historyPanel.find('tbody');
        ...
        const renderDownloadHistory = () => {
            const history = loadDownloadHistory();
            if (!history.length) {
                historyTableBody.html('<tr><td colspan="6">Chưa có dữ liệu.</td></tr>');
                return;
            }
            const rows = history.map((entry) => {
                const statusKey = (entry.status || '').toString();
                const statusClass = 'nd-history-status-' + statusKey.replace(/[^a-z-]/gi, '').toLowerCase();
                const statusLabel = historyStatusLabels[statusKey] || statusKey;
                let chapterText = '-';
                if (typeof entry.totalChapters === 'number') {
                    const success = typeof entry.successChapters === 'number' ? entry.successChapters : '?';
                    chapterText = success + ' / ' + entry.totalChapters;
                } else if (typeof entry.successChapters === 'number') {
                    chapterText = String(entry.successChapters);
                }
        ...
        container.find('[name="buttons"]').find('input[name="toggle-history"]').on('click', () => {
            if (historyPanel.is(':visible')) {
                historyPanel.hide();
            } else {
                renderDownloadHistory();
        ...
        historyPanel.find('button[name="history-close"]').on('click', () => historyPanel.hide());
        historyPanel.find('button[name="history-clear"]').on('click', () => {
            if (window.confirm('Xóa toàn bộ lịch sử tải?')) {
                saveDownloadHistory([]);
                renderDownloadHistory();
            }
        });
        const computeChapterStats = () => {
            const chaptersList = (Storage.book && Array.isArray(Storage.book.chapters)) ? Storage.book.chapters : [];
            const chaptersWithUrl = chaptersList.filter((chapter) => chapter && chapter.url);
            const successCount = chaptersWithUrl.filter((chapter) => chapter && chapter.url && (chapter.contentRaw || chapter.content)).length;
            return {
                total: chaptersWithUrl.length,
        ...
        container.find('[name="config"]').find('input,select,textarea').on('change', function (e) {
            ...
            if (name === 'downloadMode') {
                applyDownloadModeUIState();
            }
            if (['retry', 'timeout'].includes(name)) {
                xhr.storage.config.set(name, value);
            }
            if (name === 'thread' || name === 'downloadMode') {
                syncXhrThreadConfig();
                if (name === 'thread' && Config.downloadMode === 'sequential') {
                    applyDownloadModeUIState();
                }
            }
        }).each(function (i, e) {
        ...
        function openNDDB() {
            return new Promise((resolve, reject) => {
                const req = indexedDB.open('novel-downloader', 1);
        ...
        // Khi mở UI lần đầu: nếu Config.rememberDownloadDir = true thì thử load handle từ IDB
        ...
        container.find('[name="buttons"]').find('[name="choose-download-dir"]').on('click', async () => {
            if (typeof window.showDirectoryPicker !== 'function') {
                alert('Trình duyệt hiện không hỗ trợ chọn thư mục trực tiếp (File System Access API). Sẽ dùng download mặc định.');
                return;
            }
            try {
                const dirHandle = await window.showDirectoryPicker();
                Storage.downloadDirHandle = dirHandle;
                updateDownloadDirUI();
        ...
        container.find('[name="buttons"]').find('[name="download"]').on('click', async (e) => {
            ...
            const onComplete = async (force) => {
               const failedChapters = Storage.book.chapters.filter(c => !c.contentRaw && !c.document);
                ...
                //hết
                const savedFileName = await downloadTo[format](chapters);
                if (format !== 'debug') {
                    const stats = computeChapterStats();
                    addDownloadHistoryEntry({
                        title: Storage.book.title || '',
                        format,
                        status: force ? 'force-save' : 'success',
                        totalChapters: stats.total,
                        successChapters: stats.success,
            ...
            for (let attempt = 0; attempt <= maxAttempts; attempt++) {
                ...
                if (currentRunList.download.length > 0) {
                    ...
                    const chunkSize = resolveThreadConfig();
        ...
        const style = [
            ...
            '.novel-downloader-v3 .nd-history-panel{margin:6px 10px;padding:6px;border:1px solid #b5b5b5;background:#fff;text-align:left;max-height:240px;overflow:auto;display:none;}',
            '.novel-downloader-v3 .nd-history-toolbar{display:flex;justify-content:space-between;align-items:center;margin-bottom:6px;gap:6px;}',
            '.novel-downloader-v3 .nd-history-actions{display:flex;gap:4px;}',
            '.novel-downloader-v3 .nd-history-panel table{width:100%;border-collapse:collapse;font-size:12px;}',
            '.novel-downloader-v3 .nd-history-panel th,.novel-downloader-v3 .nd-history-panel td{border:1px solid #ccc;padding:4px 6px;text-align:left;}',
            '.novel-downloader-v3 .nd-history-panel tbody tr:nth-child(odd){background-color:#f7f7f7;}',
            '.novel-downloader-v3 .nd-history-status-success{color:#27ae60;font-weight:bold;}',
            '.novel-downloader-v3 .nd-history-status-failed{color:#e74c3c;font-weight:bold;}',
            '.novel-downloader-v3 .nd-history-status-force-save{color:#f39c12;font-weight:bold;}',

            '.novel-downloader-v3{all:initial;display:block;position:fixed;left:50%;top:50%;transform:translate(-50%,-50%);z-index:99999;background:#fff;border:1px solid #000;max-height:99vh;overflow:auto;text-align:center;font-family:"Segoe UI",Arial,Helvetica,sans-serif;color:#000;}',
            '.novel-downloader-v3 *{font-family:inherit;color:inherit;box-sizing:border-box;}',
    ...
    const downloadTo = {
        ...
        text: async (chapters) => {
            ...
            const fileName = `${title}__${writer}.txt`;
            ...
            if (Config.image) {
                ...
                if (Storage.book.image.filter((i) => !i.content).length) {
                    await new Promise((resolve, reject) => {
                        xhr.init({
                            retry: Config.retry,
                            thread: Storage.getEffectiveThread ? Storage.getEffectiveThread() : Math.max(1, Config.thread || 1),
                            timeout: Config.timeout * 10,
                            onComplete: () => {
                                xhr.hideDialog();
                                resolve();
  ...
  */
    ...
    async function download(content, name, force) {
        ...
        // Thử lưu bằng File System Access API nếu đã có handle
        if (Storage.downloadDirHandle) {
            try {
                // Hàm helper để tìm tên file khả dụng (ví dụ: file (1).txt)
                async function findAvailableFileName(dirHandle, originalName) {
                    ...
                    while (true) {
                        ...
                        try {
                            await dirHandle.getFileHandle(newName); // Thử lấy handle
                            counter++; // Tên đã tồn tại
                        } catch (e) {
                            if (e.name === 'NotFoundError') return newName; // Tên khả dụng
                            throw e; // Lỗi khác
                        }
                    }
                }
                ...
                let finalName = name;
                let action = 'save'; // Mặc định là lưu mới

                // Bước 1: Kiểm tra file tồn tại
                try {
                    await Storage.downloadDirHandle.getFileHandle(name);

                    // Nếu không lỗi -> file đã tồn tại. Hiển thị hộp thoại tùy chỉnh.
                    const choice = await showCustomConfirm(
                        `Tệp "${name}" đã tồn tại. Bạn muốn làm gì?`,
                        ['Ghi đè', 'Đổi tên', 'Hủy']
                    );
                    ...
                    // Nếu là NotFoundError -> file không tồn tại, cứ lưu bình thường
                }

                // Bước 2: Thực hiện hành động
                if (action === 'cancel') {
                    ndShowToast(`Đã hủy lưu tệp: ${name}`, 'info', 3000);
                    return;
                }

                if (action === 'rename') {
                    finalName = await findAvailableFileName(Storage.downloadDirHandle, name);
                }

                // Bước 3: Ghi file
                const perm = await Storage.downloadDirHandle.queryPermission({ mode: 'readwrite' });
                if (perm !== 'granted') {
                    const reqPerm = await Storage.downloadDirHandle.requestPermission({ mode: 'readwrite' });
                    if (reqPerm !== 'granted') throw new Error("Không được cấp quyền ghi vào thư mục.");
                }

                const fileHandle = await Storage.downloadDirHandle.getFileHandle(finalName, { create: true });
                const writable = await fileHandle.createWritable();
                await writable.write(content);
                await writable.close();

                if (action === 'overwrite') {
                    ndShowToast(`Đã ghi đè tệp: ${finalName}`, 'success', 4500);
                } else if (action === 'rename') {
                    ndShowToast(`Đã lưu tệp với tên mới: ${finalName}`, 'success', 4500);
                } else {
                    ndShowToast('Đã lưu vào thư mục đã chọn: ' + (Storage.downloadDirHandle?.name ? Storage.downloadDirHandle.name + '/' : '') + finalName, 'success', 4500);
                }
                return;

            } catch (err) {
                console.warn('Lỗi khi ghi bằng File System API, chuyển sang saveAs', err);
                ndShowToast(`Lỗi: ${err.message}. Chuyển sang chế độ tải xuống mặc định.`, 'error', 5000);
            }
        }

        // Fallback: gọi saveAs
        ndShowToast('Mở hộp lưu của trình duyệt (Save As)...', 'info', 2500);
        saveAs(content, name);
    }
