...
// @downloadURL  https://raw.githubusercontent.com/BaoBao666888/Novel-Downloader5/main/Fanqie%20Chapter%20Replacer%20and%20copy%20Full.user.js
// @updateURL    https://raw.githubusercontent.com/BaoBao666888/Novel-Downloader5/main/Fanqie%20Chapter%20Replacer%20and%20copy%20Full.user.js
...
(function () {
    ...
    function waitForToken(callback, fallback, itemId, maxTries = 20) {
        const interval = setInterval(() => {
            if (unsafeWindow.tokenOptions?.Fanqie) {
                clearInterval(interval);
                callback(unsafeWindow.tokenOptions.Fanqie, itemId);
            } else if (maxTries-- <= 0) {
                clearInterval(interval);
                fallback('http://rehaofan.jingluo.love', itemId);
            }
        }, 300);
    }
    ...
    function addCopyButton() {
        ...
        // Xoá nút copy cũ nếu có
        const existingBtn = header.querySelector('.fanqie-copy-btn');
        if (existingBtn) existingBtn.remove();

        const copyBtn = document.createElement('button');
        copyBtn.textContent = '📋 Copy nội dung + tiêu đề';
        copyBtn.className = 'fanqie-copy-btn';
    ...
    function fetchAndReplace(apiHost, itemId) {
    ...
    function getCurrentItemId() {
        return location.pathname.match(/\/reader\/(\d+)/)?.[1];
    }

    function handleChapterLoad() {
        const itemId = getCurrentItemId();
        if (!itemId) return;
        waitForToken(fetchAndReplace, fetchAndReplace, itemId);
    }

    // Khởi động lần đầu
    handleChapterLoad();

    // Theo dõi khi chuyển chương (url đổi nhưng không reload)
    let lastItemId = getCurrentItemId();
    ...
    const observer = new MutationObserver(() => {
        ...
        if (newItemId && newItemId !== lastItemId) {
            ...
            console.log('[Fanqie] Phát hiện chuyển chương:', newItemId);
        }

        const contentBox = document.querySelector('.muye-reader-content');
        if (!contentBox) return;

        const currentText = contentBox.innerText.trim();

        // Nếu nội dung vẫn thay đổi thì reset timer
        if (currentText !== lastRenderContent) {
            lastRenderContent = currentText;
            if (debounceTimer) clearTimeout(debounceTimer);

            debounceTimer = setTimeout(() => {
                console.log('[Fanqie] Render ổn định, thay nội dung');
                fetchAndReplace(unsafeWindow.tokenOptions?.Fanqie || 'http://rehaofan.jingluo.love', lastItemId);
            }, 600); // chờ 600ms sau khi nội dung không thay đổi
        }
    });
