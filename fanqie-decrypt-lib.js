...
export class FqClient {
    ...
    async getContentKeys(itemIds) {
        ...
        return await xhr.sync(`https://api5-normal-sinfonlineb.fqnovel.com/reading/reader/batch_full/v?item_ids=${itemIdsStr}&req_type=1&aid=${this.config.AID}&update_version_code=${this.config.VERSION_CODE}`, null, {
            method: 'GET',
            responseType: 'json',
            headers: {
    ...
    async getDecryptionKey() {
        ...
        const result = await xhr.sync(`https://api5-normal-sinfonlineb.fqnovel.com/reading/crypt/registerkey?aid=${this.config.AID}`, JSON.stringify(payload), {
            method: 'POST',
            headers: {
