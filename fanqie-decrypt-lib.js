...
(function () {
  ...
  class FqClient {
    ...
    async getContentKeys(itemIds) {
      ...
      const url = `https://api5-normal-sinfonlineb.fqnovel.com/reading/reader/batch_full/v?item_ids=${itemIdsStr}&req_type=1&aid=${this.config.AID}&update_version_code=${this.config.VERSION_CODE}`;
    ...
    async getDecryptionKey() {
      ...
      const url = `https://api5-normal-sinfonlineb.fqnovel.com/reading/crypt/registerkey?aid=${this.config.AID}`;
      const res = await xhr.sync(url, JSON.stringify(payload), {
        method: 'POST',
        headers: {
      ...
      const encryptedKey = Uint8Array.from(atob(res.response.data.key), c => c.charCodeAt(0));
      const decryptedKey = await this.crypto.decrypt(encryptedKey);
      this.dynamicKey = this.crypto.bytesToHex(decryptedKey);
      this.keyExpireTime = now + 3600 * 1000; // cache 1h
