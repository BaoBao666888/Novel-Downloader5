...
function enableTextSelectionAndContextMenu() {
    // 1. Ghi đè CSS để BẮT BUỘC cho phép tô đen văn bản
    const css = `
        * {
    ...
    `;
    removeElementById('tm-force-select-style');
    const styleEl = document.createElement('style');
    styleEl.id = 'tm-force-select-style';
    styleEl.textContent = css;
    document.head.appendChild(styleEl);

    // 2. Vô hiệu hóa các thuộc tính event JavaScript dùng để chặn
    const eventsToUnblock = ['oncontextmenu', 'onselectstart', 'ondragstart', 'onmousedown'];
    const allElements = document.body.querySelectorAll('*');

    allElements.forEach(el => {
        eventsToUnblock.forEach(eventName => {
            // Xóa trực tiếp các thuộc tính HTML (vd: <body oncontextmenu="...">)
            if (el.hasAttribute(eventName)) {
                el.removeAttribute(eventName);
            }
            // Ghi đè các thuộc tính được gán bằng JS (vd: element.oncontextmenu = function...)
            if (el[eventName]) {
                el[eventName] = null;
            }
        });
    ...
    console.log('[tm-translate] Đã kích hoạt lại việc chọn văn bản và chuột phải.');
}
...
async function startTranslateAction() {
    try {
        ...
        enableTextSelectionAndContextMenu();
