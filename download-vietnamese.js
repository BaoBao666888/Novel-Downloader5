...
(function (window) {
  const storageInit = {
    default: {
      ...
      css: [
        ...
        '#gmDownloadDialog>.task>div[status="downloading"]>[name="status"]::before{content:"Đang tải";color:#00f;}',
        '#gmDownloadDialog>.task>div[status="error"]>[name="status"]::before{content:"Lỗi";color:#f00;}',
        '#gmDownloadDialog>.task>div[status="timeout"]>[name="status"]::before{content:"Quá giờ";color:#f00;}',
        '#gmDownloadDialog>.task>div[status="abort"]>[name="status"]::before{content:"Bị hủy";color:#f00;}',
  ...
  main.init = (option) => {
    ...
    dialog.innerHTML = [
      ...
      '  <progress name="total-progress" value="0" max="1" title="Nhấp để xóa mục đã hoàn thành"></progress>',
  ...
  function xhrSync (url, data = null, opt = {}) {
    return new Promise((resolve, reject) => {
      const optRaw = Object.assign({}, opt);
      opt.onload = (res) => {
